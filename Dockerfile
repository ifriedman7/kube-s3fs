...
CMD echo "${AWS_KEY}:${AWS_SECRET_KEY}" > /etc/passwd-s3fs && \
    ...
    /usr/bin/s3fs $S3_BUCKET $MNT_POINT -f -o endpoint=${S3_REGION} -o allow_other -o use_cache=/tmp -o max_stat_cache_size=1000 -o stat_cache_expire=900 -o retries=5 -o connect_timeout=10 -o nonempty -o noua -o use_path_request_style
